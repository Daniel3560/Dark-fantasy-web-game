...
let bossFlashTimer = 0; // Timer for boss flash effect
...
// --- Asset Loading: Add Bat Image ---
const batImage = new Image();
batImage.src = 'bat.png'; // Make sure you have a bat.png in your project
batImage.onload = () => {
    console.log("bat.png loaded successfully!");
    updateAssetStatus('bat', true);
       };
batImage.onerror = () => {
    console.error("Failed to load bat.png!");
    updateAssetStatus('bat', false);
};

// --- Asset Loading: Add Medusa Image ---
const medusaImage = new Image();
medusaImage.src = 'medusa.png'; // Make sure you have a medusa.png in your project
medusaImage.onload = () => {
    console.log("medusa.png loaded successfully!");
    // Optionally, add to assetStatus if you want to track Medusa image loading
};
medusaImage.onerror = () => {
    console.error("Failed to load medusa.png!");
};
...
let player = {
    ...
    width: 80, // Knight's width for drawing and collision (adjust to your knight.png size)
    height: 100, // Knight's height for drawing and collision (adjust to your knight.png size)
    ...
    baseAttackDamage: 100, // NEW: Base attack damage
...
const ALL_MONSTERS_DEFINITION = [
    // Common Skeleton types (attack damage now 15)
        { x: 300, yOffsetFromMainGround: 0, width: 100, height: 150, health: 20, type: 'Skeleton Warrior', loot: 'Bone Shard', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 100, startX: 300 },
    { x: 2100, yOffsetFromMainGround: 0, width: 110, height: 160, health: 20, type: 'Skeleton Guard', loot: 'Rusty Sword', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 120, startX: 2100 },
    { x: 2900, yOffsetFromMainGround: 0, width: 100, height: 150, health: 20, type: 'Armored Skeleton', loot: 'Broken Plate', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 90, startX: 2900 },
    { x: 4200, yOffsetFromMainGround: 0, width: 100, height: 150, health: 20, type: 'Skeleton Knight', loot: 'Knightly Helm', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 110, startX: 4200 },
    { x: 500, yOffsetFromMainGround: -150, width: 100, height: 150, health: 20, type: 'Risen Archer', loot: 'Rotten Arrow', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 80, startX: 500 },
    { x: 3600, yOffsetFromMainGround: -100, width: 100, height: 150, health: 20, type: 'Skeleton Archer', loot: 'Old Bow', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 100, startX: 3600 },
    { x: 4800, yOffsetFromMainGround: -50, width: 100, height: 150, health: 20, type: 'Skeleton Mage', loot: 'Dusty Tome', isSkeleton: true, color: '#C2B280', velX: 0.5, velY: 0, isJumping: true, jumpForce: -7, speed: 1.5, attackDamage: 15, attackCooldown: 1000, lastAttackTime: 0, patrolRange: 95, startX: 4800 },
    // Unique Level 1 Monsters (Shadow Weaver attack damage now 25)
    { x: 1300, yOffsetFromMainGround: -120, width: 120, height: 80, health: 50, type: 'Shadow Weaver', loot: 'Whispering Silk', isSkeleton: false, color: '#4B0082', image: shadowWeaverImage, velX: 0.7, velY: 0, isJumping: true, jumpForce: -8, speed: 2, attackDamage: 25, attackCooldown: 1500, lastAttackTime: 0 },
    ...
    const BAT_DEFINITION = [
    { x: 200, yOffsetFromMainGround: -250, width: 100, height: 80, health: 15,type: 'Bat', loot: 'Bat Wing', isSkeleton: false, color: '#444',isBat: true, speed: 3, flyRange: 200, flyCenterY: -250, direction: 1, angle: 0, startX: 800},
    { x: 400, yOffsetFromMainGround: -200, width: 100, height: 80, health: 15,type: 'Bat', loot: 'Bat Wing', isSkeleton: false, color: '#444',isBat: true, speed: 2.5, flyRange: 180, flyCenterY: -200, direction: 1, angle: Math.PI / 2, startX: 2000},
    ];

    const MEDUSA_BOSS_DEFINITION = { 
    
        x: 3200, yOffsetFromMainGround: -120, width: 160, height: 180, health: 150, type: 'Medusa', loot: 'Medusa Head', isSkeleton: false, isBat: false, isBoss: true, color: '#228B22', speed: 2, attackDamage: 30, attackCooldown: 1200, lastAttackTime: 0, patrolRange: 0, startX: 3200 };
...
gameState = 'playing';
        ...
        player.x = 100; // Spawn player on the left side for round 2
    ...
    // --- Add bats for round 2 (OUTSIDE the forEach loop!) ---
    BAT_DEFINITION.forEach(batDef => {
        let newBat = JSON.parse(JSON.stringify(batDef));
        newBat.y = mainGroundY + newBat.yOffsetFromMainGround - newBat.height;
        newBat.x = newBat.startX;
        if (typeof newBat.angle !== "number") newBat.angle = 0; 
        monstersToSpawn.push(newBat);
    });
     gameState = 'playing';
}
        
else if (level === 3) {
    player.x = 100; // Spawn player on the left side for round 3

    // Only add the original monsters (no duplicates)
    ALL_MONSTERS_DEFINITION.forEach(monsterDef => {
        let newMonster = JSON.parse(JSON.stringify(monsterDef));
        newMonster.health = maxMonsterHealth(newMonster.type);
        monstersToSpawn.push(newMonster);
    });

    // Add bats for round 3 (optional, same as round 2)
    BAT_DEFINITION.forEach(batDef => {
        let newBat = JSON.parse(JSON.stringify(batDef));
        newBat.y = mainGroundY + newBat.yOffsetFromMainGround - newBat.height;
        newBat.x = newBat.startX;
       ...
       // --- Add Medusa Boss for round 3 only ---
    let medusa = JSON.parse(JSON.stringify(MEDUSA_BOSS_DEFINITION));
    medusa.y = mainGroundY + medusa.yOffsetFromMainGround - medusa.height;
    monstersToSpawn.push(medusa);

     // --- Trigger boss flash ---
    bossFlashTimer = 90; // Number of frames to flash (about 1 second at 60fps)
    gameState = 'bossFlash';
}
...
const MAX_INVENTORY_ITEMS = 10; // NEW: Maximum number of items in inventory
...
function update() {
    ...
    console.log("All monsters defeated! Triggering round complete message.");
    gameState = 'levelComplete'; // Show round complete message

    setTimeout(() => {
        // After 2 seconds, respawn monsters and let spawnMonstersForLevel handle the state
        currentLevel++;
        spawnMonstersForLevel(currentLevel);
        // Do NOT set gameState here! Let spawnMonstersForLevel handle it for bossFlash.
    ...
    monsters.forEach(monster => {
          // --- Bat flying logic (sinusoidal movement) ---
    if (monster.isBat) {
    ...
    // Optional: Bat attack logic (if close enough)
    const batAttackRange = 40;
    if (distance < batAttackRange) {
        const currentTime = Date.now();
        if (!monster.lastAttackTime || currentTime - monster.lastAttackTime > 1000) {
            player.health -= 10; // Bat attack damage
            monster.lastAttackTime = currentTime;
            playerPainSound.currentTime = 0;
            playerPainSound.play().catch(() => {});
        }
    }

    return; // Skip ground AI and gravity for bats
}


// --- Medusa always chases the player ---
if (monster.type === 'Medusa') {
    const dx = (player.x + player.width / 2) - (monster.x + monster.width / 2);
    const dy = (player.y + player.height / 2) - (monster.y + monster.height / 2);
    const distance = Math.sqrt(dx * dx + dy * dy);

    // Move Medusa toward the player
    if (distance > 1) {
        monster.x += (dx / distance) * monster.speed;
        monster.y += (dy / distance) * (monster.speed * 0.7); // Slightly slower vertical movement
    }

    // Medusa attack if close enough
    const attackRange = 50;
    if (distance < attackRange) {
        const currentTime = Date.now();
        if (!monster.lastAttackTime || currentTime - monster.lastAttackTime > monster.attackCooldown) {
    ...
    return; // Skip ground AI and gravity for Medusa
}
...
function drawMonsters() {
    monsters.forEach(monster => {
        ...
        if (
            monsterXOnCanvas + monster.width > 0 &&
            monsterXOnCanvas < canvas.width &&
            monsterYOnCanvas + monster.height > 0 &&
            monsterYOnCanvas < canvas.height
        ) {
            // --- Draw bat image if this is a bat ---
            if (monster.isBat && batImage.complete && batImage.naturalWidth > 0) {
                ctx.drawImage(batImage, monsterXOnCanvas, monsterYOnCanvas, monster.width, monster.height);
            // --- Draw Medusa image if this is Medusa ---
            } else if (monster.type === 'Medusa' && medusaImage.complete && medusaImage.naturalWidth > 0) {
                ctx.drawImage(medusaImage, monsterXOnCanvas, monsterYOnCanvas, monster.width, monster.height);
            } else {
            ...
            // --- Draw health bar (keep this as is) ---
            if (typeof monster.health !== 'undefined' && monster.health > 0) {
                const healthBarX = monsterXOnCanvas;
                let healthBarY = monsterYOnCanvas - 10; // Default for most monsters
                let barWidth = monster.width;
                let barHeight = 5;

                if (monster.type === 'Shadow Weaver') {
                    healthBarY = monsterYOnCanvas - 55;
                } else if (monster.isSkeleton) {
                    healthBarY = monsterYOnCanvas + 10;
                    barWidth = monster.width * 0.6;
                    barHeight = 3;
                } else if (monster.isBat) {
                    healthBarY = monsterYOnCanvas - 15;
                } else if (monster.type === 'Medusa') {
                    barWidth = monster.width * 0.2;
                    barHeight = 7;
                    healthBarY = monsterYOnCanvas - 20;
                }

                ctx.fillStyle = '#8B0000'; // Dark red for background of health bar (empty health)
                ctx.fillRect(healthBarX + (monster.width - barWidth) / 2, healthBarY, barWidth, barHeight);

                ctx.fillStyle = 'red'; // Red for current health
                const healthBarWidth = (monster.health / maxMonsterHealth(monster.type)) * barWidth;
                ctx.fillRect(healthBarX + (monster.width - barWidth) / 2, healthBarY, healthBarWidth, barHeight);
            }
        } 
    }); 
} 
...
if (gameState === 'playing' || gameState === 'levelComplete') {
        ...
        const shopIconSize = 120; // Increased size
...
//draw round complete overlay
        ...
        ctx.font = '40px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('Round Complete!', canvas.width / 2, canvas.height / 2);
        ctx.font = '20px monospace';
        ctx.fillStyle = 'white';
        ctx.fillText('Get ready for the next wave...', canvas.width / 2, canvas.height / 2 + 40);
        ctx.textAlign = 'left';

}

    // --- Boss Flash Overlay ---
    if (gameState === 'bossFlash') {
        ctx.fillStyle = 'rgba(255,255,255,0.85)';
        ctx.fillRect(0, 0, canvas.width, canvas.height);
        ctx.fillStyle = '#228B22';
        ctx.font = 'bold 60px monospace';
        ctx.textAlign = 'center';
        ctx.fillText('MEDUSA APPROACHES!', canvas.width / 2, canvas.height / 2);
        ctx.textAlign = 'left';
    }
}
